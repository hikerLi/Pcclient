...
#include <Common/concurrent_priority_queue.h>
#include <mutex>
...
//#define TimerIns Timer::GetInstance()
...
uint64_t Timer::AddTimes(uint64_t milliseconds, int32_t times, FunctionType&& func)
{
    ...
    TimerTask * task = new TimerTask(timerId, milliseconds, times, std::forward<FunctionType>(func));
